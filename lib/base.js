...
define(
  ...
  function(shim, utils, registry, debug) {
    'use strict';
    ...
    function withBase() {
      ...
      this.trigger = function() {
        ...
        if (typeof this.attr.eventData === 'object') {
          data = shim.extend({}, this.attr.eventData, data, true);
        }

        shim.trigger($element, (event || type), data);

        if (defaultFn && !shim.isDefaultPrevented(event)) {
      ...
      this.off = function() {
        ...
        return shim.off($element, type, callback);
      };
      ...
      this.select = function(attributeKey) {
        return shim.find(this.$node, this.attr[attributeKey]);
      };
      ...
      this.initialize = function(node, attrs) {
        ...
        if (shim.isWrapped(node)) {
          this.node = shim.unwrap(node);
          this.$node = node;
        } else {
          this.node = node;
          this.$node = shim.wrap(node);
        }
